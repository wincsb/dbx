...
func Open(driverName, dataSourceName string) (*DB, error) {
	...
	return &DB{
		...
		tableData:        make(map[string]*sync.Map),	// 第一级的 map 会在启动的时候初始化好，第二级的使用安全 map
...
func (db *DB) ErrorSQL(s string, sql1 string, args... interface{}) {
	...
	fmt.Fprintf(db.Stderr, "[%v] %v\n", time.Now().Format("2006-01-02 15:04:05"), sql2, str)
}
...
func (db *DB) BindStruct(tableName string, ifc interface{}, enableCache bool) {
...
func (q *Query) toSQL(tableStruct *TableStruct, action int, rvalues ...reflect.Value) (sql1 string, args []interface{}) {
	...
	case ACTION_INSERT:
		...
		args = struct_value_to_args(tableStruct, rvalues[0], true)
	...
	case ACTION_REPLACE:
		...
		args = struct_value_to_args(tableStruct, rvalues[0], false)
	case ACTION_COUNT:
...
func (q *Query) One(arrIfc interface{}) (err error) {
	...
	// 如果没有 BindStruct() ，这里就会执行下去，从缓存里读表结构，不用每次都反射，提高效率
...
func (q *Query) Insert(ifc interface{}) (insertId int64, err error) {
	return q.insert_replace(ifc, false)
}

// ifc 最好为 &struct
func (q *Query) Replace(ifc interface{}) (insertId int64, err error) {
	return q.insert_replace(ifc, true)
}

func (q *Query) insert_replace(ifc interface{}, isReplace bool) (insertId int64, err error) {
	if q.readOnly {
		return
	}
...
func (q *Query) Update(ifc interface{}) (affectedRows int64, err error) {
	...
	if arrTypeElem.Kind() != reflect.Struct {
		err = errors.New(fmt.Sprintf("must pass a struct or struct pointer: %v", arrTypeElem.Kind()))
		return
	}
