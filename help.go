...
func get_pk_from_sql(sql string) []string {
	reg := regexp.MustCompile(`(?i)primary\s+key.*?\((.*?)\)`)
	arr := reg.FindStringSubmatch(sql)
	if len(arr) < 2 {
		reg = regexp.MustCompile(`(?i)(\w+).*(?:primary key)`)
		arr := reg.FindStringSubmatch(sql)
		if len(arr) == 2 {
			return []string{arr[1]}
		} else {
...
// uncludePK 是否排除主键
func struct_value_to_args(tableStruct *TableStruct, value reflect.Value, uncludePK bool) ([]interface{}) {
